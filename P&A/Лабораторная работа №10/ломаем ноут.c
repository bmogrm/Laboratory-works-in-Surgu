...
int main(){
	...
	do {
		...
		switch(choose) {
			case 1:
				while(1){
					t = (baza*)realloc(event, (q+1) * sizeof(baza));
					if (t != NULL) event = t;
					else {
					...
					input(event, q);
					q++;
					printf("Всего структур создано - %d\n", q);
				}
				break;
			case 2:
				output(event, q);
			...
			case 7:
				q = del(event, q);
				t = (baza*)realloc(event, q * sizeof(baza));
				if (t != NULL) event = t;
				else {
					printf("Ошибка выделения памяти!");
					break;
	...
	free(event);
	event = NULL; // Отчистка памяти перед завершением работы программы
...
//	printf("\n\n");
	...
	event[num].day_start = 13;
	event[num].day_stop = 13;
	event[num].mounth_start = 03;
	event[num].year_start = 2023;
	event[num].hours_start = 12;
	event[num].min_start = 00;
	event[num].mounth_stop = 03;
	event[num].year_stop = 2023;
	event[num].hours_stop = 13;
	event[num].min_stop = 00;
}
...
int loadStruct(baza **event, int *num){
	...
	while(1){
		...
		*event = (baza*)realloc(*event, (i+1) * sizeof(baza));
		if(*event == NULL) {
			printf("Ошибка выделения памяти!");
			break;
		}
		
		if (fscanf(fl, "%d%d%d", &(*event)[i].day_start, &(*event)[i].mounth_start, &(*event)[i].year_start) != 3) {
			printf("Файл успешно загружен!\n");
			break;
		}
		if (fscanf(fl, "%d%d\n", &(*event)[i].hours_start, &(*event)[i].min_start) != 2) {
			printf("Файл успешно загружен!\n");
			break;
		}
		
		if (fscanf(fl, "%d%d%d", &(*event)[i].day_stop, &(*event)[i].mounth_stop, &(*event)[i].year_stop) != 3) {
			printf("Файл успешно загружен!\n");
			break;
		}
		if (fscanf(fl, "%d%d\n", &(*event)[i].hours_stop, &(*event)[i].min_stop) != 2) {
			printf("Файл успешно загружен!\n");
			break;
		}
