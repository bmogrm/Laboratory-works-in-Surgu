...
void print(const struct Visitor *p)
{
	...
	/*printf("Birthday:\t%d.%d.%d\n", p->birthday.d, p->birthday.m, p->birthday.y);*/
	printf("Diagnos:\t%s\n", p->reason);
	printf("Povtor:\t%s\n", (p->secondary ? "yes" : "no"));
...
int is_child(const struct Visitor *p)
{
	return (2020 - p->birthday.y) < 18;
}
...
/*
		...
		Т.Е.: пишем функцию - сразу здесь ее вызываем, чтобы проверить, и т.д.
*/
...
/*
		В лабораторной работе №8 будут использоваться эти функции, например в
		цикле используется if (comp_fio(&a[i], &a[i+1])) чтобы сравнить 2 элемента
		(посетителя) при сортировке, или функции is_child и print чтобы определить
		(не)совершеннолетних и вывести на экран при поиске и т.д.
*/
